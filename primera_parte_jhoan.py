...
def continuar_incendio():
  ...
  for v in range(len(bosque)):
    if bosque[v].state == 'ENCENDIDO':
      if random.random() <= p_muerte:
        bosque[v].state = 'MUERTO'
  ...
  for w in range (len(bosque)):
    if bosque[w].state == 'ENCENDIDO':
...
# x = 800, p_base = 0.8 p_muerte = 0.15 seed = 123456789  da un resultado interesante
